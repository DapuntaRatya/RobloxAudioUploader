...
from app.modules.roblox.asset_upload import create_audio_asset, sanitize_roblox_asset_name
...
permission_semaphore: asyncio.Semaphore | None = None


def init_semaphores(max_download: int, max_upload: int, max_permission: int) -> None:
    global download_semaphore, upload_semaphore, permission_semaphore
    download_semaphore = asyncio.Semaphore(max_download)
    upload_semaphore = asyncio.Semaphore(max_upload)
    permission_semaphore = asyncio.Semaphore(max_permission)


def safe_display_name(title: str, fallback: str) -> str:
    return sanitize_roblox_asset_name(title or fallback, fallback=fallback)
...
class UploadOrchestrator:
    ...
    async def process_job(self, job_id: str) -> None:
        ...
        if not session:
            await self.jobs.mark_job_failed(job_id)
